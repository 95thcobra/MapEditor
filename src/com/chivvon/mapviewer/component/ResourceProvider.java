...
public final class ResourceProvider extends Provider implements Runnable {
	...
	private final int MAX_ONDEMAND_CHUNK_LENGTH_BYTES = 500;
	...
	public ResourceProvider() {
		requested = new Deque();
		loadingMessage = "";
		payload = new byte[MAX_ONDEMAND_CHUNK_LENGTH_BYTES];
		...
		try {
			...
			if (remainingData == 0 && available >= 10) {
				...
				if (current != null) {
					...
					idleTime = 0;
					if (length == 0) {
						System.err.println("Rej: " + type + "," + file);
						current.buffer = null;
				...
				completedSize = sector * MAX_ONDEMAND_CHUNK_LENGTH_BYTES;
				remainingData = MAX_ONDEMAND_CHUNK_LENGTH_BYTES;
				if (remainingData > length - sector * MAX_ONDEMAND_CHUNK_LENGTH_BYTES)
					remainingData = length - sector * MAX_ONDEMAND_CHUNK_LENGTH_BYTES;
			}
			if (remainingData > 0 && available >= remainingData) {
				...
				if (remainingData + completedSize >= data.length && current != null) {
					if (View.cache.getStore(0) != null)
						View.cache.getStore(current.dataType + 1).writeFile(data.length, data, current.ID);
	...
	public void initialize(Archive archive, View client) {
	...
	public void run() {
		try {		
			while (running) {
				tick++;
				int sleepTime = 20;
				if (maximumPriority == 0 && View.cache.getStore(0) != null)
					sleepTime = 50;
				try {
	...
	public void requestExtra(byte priority, int type, int file) {
		...
		byte[] data = View.cache.getStore(type + 1).readFile(file);
	...
	private void loadMandatory() {
		...
		while (resource != null) {
			...
			if (View.cache.getStore(0) != null)
				data = View.cache.getStore(resource.dataType + 1).readFile(resource.ID);
	...
	public int getChecksum(int type, int id) {
		...
		byte[] data = View.cache.getStore(type + 1).readFile(id);
	...
	public void writeChecksumList(int type) {
		try {
			DataOutputStream out = new DataOutputStream(new FileOutputStream(View.cache.getRoot().toString() + type + "_crc.dat"));
			int total = 0;
			for (int index = 0; index < View.cache.getStore(type + 1).getFileCount(); index++) {
	...
	public void writeVersionList(int type) {
		try {
			DataOutputStream out = new DataOutputStream(new FileOutputStream(View.cache.getRoot().toString() + type + "_version.dat"));
			for (int index = 0; index < View.cache.getStore(type + 1).getFileCount(); index++) {
